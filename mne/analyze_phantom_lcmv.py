...
def evaluate_dipole_fit(dipole_amplitude, show=False,
    ...
    for dipole_idx in dipole_indices:
        ...
        # t_peak = 66e-3  # ~60 MS at largest peak
        noise_cov = mne.compute_covariance(epochs, tmax=-0.05)
        data_cov = mne.compute_covariance(epochs, tmin=0.)

        # stc = lcmv(evoked, fwd, noise_cov, data_cov, reg=0.05, pick_ori='free')
        stc = lcmv(evoked, fwd, noise_cov, data_cov, reg=0.01,
                   # pick_ori='max-power', weightnorm=None)
                   pick_ori='max-power', weightnorm='unitnoisegain')

        # idx_peak = np.where(stc.times > t_peak)[0][0]
        # norms = stc.data[:, idx_peak]
        # norms[np.isnan(norms)] = 0.
        # idx_max = np.argmax(norms)
        idx_max = np.argmax(np.max(stc.data, axis=1))
...
for use_maxwell_filter in [True, False, 'mne']:
    ...
    plt.savefig('phantom_errors_%s%s.pdf' % (method, suffix))
